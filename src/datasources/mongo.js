...
class mongoAPI extends DataSource {
  constructor({ mongoInstance }) {
    ...
    this.XpressDoxReturn = mongoInstance.XpressDoxReturn;
  }
  ...
  //XpressDoxReturns
  async xpressDoxReturn(id) {
    try {
      const xpressDoxReturn = await this.XpressDoxReturn.findOne({ _id: id });
      return xpressDoxReturn ? xpressDoxReturn : null;
    } catch (e) {
      console.log('Oops Something went wrong with finding the xpressDoxReturn');
      throw new ApolloError(e.message, 'ACTION_NOT_COMPLETED', {});
    }
  }

  async xpressDoxReturns(limit = 10, skip = 0, query = {}) {
    try {
      const count = await this.XpressDoxReturn.where(query).countDocuments();
      if (skip >= count) {
        skip = 0;
      }
      const records = await this.XpressDoxReturn.find(query).limit(limit).skip(skip).sort({ createdAt: -1 });
      return records.length > 0 ? { records, count } : { records: [], count: 0 };
    } catch (e) {
      console.log('Oops Something went wrong', e);
      throw new ApolloError(e.message, 'ACTION_NOT_COMPLETED', {});
  ...
  async createXpressDoxReturn(args) {
    try {
      const xpressDoxReturn = await this.XpressDoxReturn.create(args);
      return xpressDoxReturn ? xpressDoxReturn : null;
    } catch (e) {
      console.log('Oops Something went wrong', e);
      throw new ApolloError(e.message, 'ACTION_NOT_COMPLETED', {});
  ...
  async updateXpressDoxReturn(args) {
    try {
      const id = args.id
      const updatedXpressDoxReturn = await this.XpressDoxReturn.findOneAndUpdate({ _id: id }, args, { new: true });
      return updatedXpressDoxReturn ? { id, updated: true, xpressDoxReturn: updatedXpressDoxReturn } : { id, updated: false, xpressDoxReturn: null };
    } catch (e) {
      console.log('Oops Something went wrong', e);
      throw new ApolloError(e.message, 'ACTION_NOT_COMPLETED', {});
    }
  ...
  async deleteXpressDoxReturn(id) {
    try {
      const deletedXpressDoxReturn = await this.XpressDoxReturn.deleteOne({ _id: id });
      return deletedXpressDoxReturn.deletedCount > 0 ? { id, deleted: true, xpressDoxReturn: deletedXpressDoxReturn } : { id, deleted: false, xpressDoxReturn: null };
    } catch (e) {
      console.log('Oops Something went wrong', e);
      throw new ApolloError(e.message, 'ACTION_NOT_COMPLETED', {});
    }
