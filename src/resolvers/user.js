...
const {
  ...
  getPasswordHash,
  getJWT,
  getUserToUserMailTemplate,
  getUserOnboardingMailTemplate,
  getShopEnquiryTemplate,
} = require("../utils");
...
module.exports = {
  Query: {
    ...
    users: async (parent, args, context, info) => {
      const users = await context.dataSources.mongoAPI.users(
        args.limit,
        args.skip,
        args.query
      );
  ...
  Mutation: {
    login: async (parent, args, context, info) => {
      ...
      const user = await context.dataSources.mongoAPI.findUserbyEmail(
        args.email
      );
      ...
      if (!passwordMatch)
        throw new AuthenticationError("The password you supplied is invalid!");
    ...
    getResetPasswordLink: async (parent, args, context, info) => {
      ...
      var job = context.dataSources.mailAPI.sendMail(
        user.email,
        "Password Reset",
        populatedTemplate
      );
    ...
    resetPassword: async (parent, args, context, info) => {
      ...
      const user = await context.dataSources.mongoAPI.findUserbyResetToken(
        args.resetToken
      );
      if (!user)
        throw new UserInputError(
          "Could not validate this reset link. Please make sure that you have clicked on the most recent password reset link that was sent to you."
        );
    ...
    sendUserToUserEmailMessage: async (parent, args, context, info) => {
      ...
      if (!toUser || !fromUser)
        throw new UserInputError("Could not find one of the users specified!");
      const populatedTemplate = await getUserToUserMailTemplate(
        toUser,
        fromUser,
        args.message
      );
      var job = context.dataSources.mailAPI.sendMail(
        toUser.email,
        `NLA Message From ${fromUser.firstName} ${fromUser.lastName}`,
        populatedTemplate
      );
      if (job) return job;
      throw new ApolloError(
        "Could not generate user to user message mailer",
        "ACTION_NOT_COMPLETED",
        {}
      );
    },
    sendOTP: async (parent, args, context, info) => {
      const sendOTPResponse = await context.dataSources.notificationAPI.sendSMS(
        args.recipient,
        args.message
      );
      if (!sendOTPResponse)
        throw new ApolloError("Could not send OTP", "ACTION_NOT_COMPLETED", {});
      return sendOTPResponse;
    },
    sendShopEnquiry: async (parent, args, context, info) => {
      const fromUser = await context.dataSources.mongoAPI.user(args.from);
      if (!fromUser) throw new UserInputError("Could not find user specified!");
      const populatedTemplate = await getShopEnquiryTemplate(
        fromUser,
        args.items
      );
      var job = context.dataSources.mailAPI.sendMail(
        process.env.APP_SHOP_ENQUIRY_MAILER_TO_ADDRESS,
        `NLA Shop Enquiry From ${fromUser.firstName} ${fromUser.lastName}`,
        populatedTemplate
      );
      if (job) return job;
      throw new ApolloError(
        "Could not generate shop enquiry mailer",
        "ACTION_NOT_COMPLETED",
        {}
      );
    },
    createUser: async (parent, args, context, info) => {
      ...
      if (user) {
        ...
        var job = context.dataSources.mailAPI.sendMail(
          process.env.APP_USER_CREATED_MAILER_TO_ADDRESS,
          `Van Deventers & Van Deventers Has A New Customer`,
          populatedTemplate
        );
        const token = await getJWT(user);
        return { user, token };
